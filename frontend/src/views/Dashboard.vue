  ...
  <div class="space-y-8">
    ...
    <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
      <div>
        ...
        <p class="text-slate-400 text-sm mt-1">Here is your culinary overview for today.</p>
      </div>
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
      ...
      <div class="p-6 rounded-2xl glass-card relative overflow-hidden">
        <div class="absolute top-4 right-4 text-brand-400/20">
          <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-12 h-12">
            <path stroke-linecap="round" stroke-linejoin="round" d="M20.25 7.5l-.625 10.632a2.25 2.25 0 01-2.247 2.118H6.622a2.25 2.25 0 01-2.247-2.118L3.75 7.5M10 11.25h4M3.375 7.5h17.25c.621 0 1.125-.504 1.125-1.125v-1.5c0-.621-.504-1.125-1.125-1.125H3.375c-.621 0-1.125 5.04-1.125 1.125v1.5c0 .621.504 1.125 1.125 1.125z" />
          </svg>
        </div>
        <p class="text-xs font-semibold text-slate-500 uppercase tracking-wider">Pantry Stock</p>
        <h3 class="text-3xl font-bold mt-2 text-slate-100">{{ pantryCount }}</h3>
        <p class="text-xs text-slate-400 mt-1">Unique ingredients stocked</p>
      </div>
      ...
      <div class="p-6 rounded-2xl glass-card relative overflow-hidden">
        ...
        <p class="text-xs font-semibold text-slate-500 uppercase tracking-wider">Total Recipes</p>
        <h3 class="text-3xl font-bold mt-2 text-slate-100">{{ recipeCount }}</h3>
        <p class="text-xs text-slate-400 mt-1">Available culinary cards</p>
      </div>
      ...
      <div class="p-6 rounded-2xl glass-card relative overflow-hidden">
        ...
        <p class="text-xs font-semibold text-slate-500 uppercase tracking-wider">Today's Meals</p>
      ...
      <div class="p-6 rounded-2xl glass-card relative overflow-hidden">
        ...
        <p class="text-xs font-semibold text-slate-500 uppercase tracking-wider">Social Actions</p>
        <h3 class="text-3xl font-bold mt-2 text-slate-100">{{ pendingRequestsCount }}</h3>
        <p class="text-xs text-slate-400 mt-1">Pending cooking requests</p>
      </div>
    </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
      ...
      <div class="lg:col-span-2 space-y-6">
        <div class="p-6 rounded-2xl glass-panel">
          ...
          <div v-else-if="todayMeals.length === 0" class="py-12 text-center text-slate-500 border border-dashed border-slate-800 rounded-xl">
            ...
            <p class="text-sm font-medium">No meals planned for today.</p>
            <router-link to="/menu" class="text-xs text-brand-400 hover:text-brand-300 font-semibold mt-1 inline-block">Schedule a meal now &rarr;</router-link>
          </div>
          ...
          <div v-else class="space-y-4">
            ...
            >
              <div class="flex items-center gap-4">
                ...
                <div>
                  ...
                  <div v-if="meal.recipe && meal.recipe.total_kbju" class="text-xs text-slate-500 mt-1 flex gap-3 font-medium">
                    <span>Cal: {{ meal.recipe.total_kbju.calories }} kcal</span>
                    <span>P: {{ meal.recipe.total_kbju.proteins }}g</span>
                    <span>F: {{ meal.recipe.total_kbju.fats }}g</span>
                    <span>C: {{ meal.recipe.total_kbju.carbohydrates }}g</span>
                  </div>
                </div>
              </div>
              ...
              <div v-if="meal.recipe && meal.recipe.smart_match" class="text-right shrink-0">
                ...
                >
                  {{ meal.recipe.smart_match.can_cook ? 'Ready' : `Missing ${100 - meal.recipe.smart_match.match_percentage}%` }}
                </span>
              </div>
            </div>
          </div>
      ...
      <div class="space-y-6">
        <div class="p-6 rounded-2xl glass-panel">
          ...
          <div v-else-if="incomingRequests.length === 0" class="py-8 text-center text-slate-500 border border-dashed border-slate-800 rounded-xl">
            <p class="text-xs">No pending requests from other users.</p>
          </div>
          ...
          <div v-else class="space-y-3">
            ...
            >
              <div class="flex justify-between">
                ...
                <span class="text-slate-500 font-medium">wants you to cook:</span>
              </div>
              <div class="font-semibold text-brand-400 text-sm">
                {{ req.recipe_title || 'Shared Menu' }}
              </div>
